//ESC 1 --- front right --- digital pin 4
//ESC 2 --- back  right --- digital pin 5
//ESC 3 --- back  left  --- digital pin 6
//ESC 4 --- front left  --- digital pin 7
...
#define LED 12
byte pin_last_state[4];                                           //last state of pins 8, 9, 10 & 11
byte eeprom_data[36], start, data;
boolean new_function_request,first_angle;
volatile int receiver_input_channel[4];                           //receiver input for roll, pitch, throttle & yaw movement
int esc_output[4];                                                //esc output
int receiver_input_pin[5];                                        //receiver input from pins 8, 9, 10 & 11
...
unsigned long timer_channel[4], esc_timer, esc_loop_timer;
unsigned long zero_timer, timer[4], current_time;
...
void setup(){
  ...
  while(eeprom_data[33] != 'J' || eeprom_data[34] != 'M' || eeprom_data[35] != 'B'){
    ...
    digitalWrite(LED, !digitalRead(LED));                                                 //Change the led status to indicate error.
  }
...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    for(vibration_counter = 0; vibration_counter < 625; vibration_counter++){           //Do this loop 625 times
      delay(3);                                                                                  //Wait 3000us.
      esc_output[1] = 1000;                                                             //Set the pulse for ESC 1 to 1000us.
      esc_output[2] = 1000;                                                             //Set the pulse for ESC 1 to 1000us.
      esc_output[3] = 1000;                                                             //Set the pulse for ESC 1 to 1000us.
      esc_output[4] = 1000;                                                             //Set the pulse for ESC 1 to 1000us.
      esc_pulse_output();                                                               //Send the ESC control pulses.
    }
    vibration_counter = 0;                                                              //Reset the vibration_counter variable.
  }

  receiver_input_channel[3] = convert_receiver_channel(3);                       //Convert the actual receiver signals for throttle to the standard .
  if(receiver_input_channel[3] < 1025)new_function_request = false;          //If throttle in the lowest position set the request flag to false.
  ...
  if(data == 0 && new_function_request == false){                                   //Only start the calibration mode at first start. 
    receiver_input_channel[3] = convert_receiver_channel(3);                  //Convert the actual receiver signals for throttle to the standard .
    esc_output[1] = receiver_input_channel[3];                                          //Set the pulse for motor 1 equal to the throttle channel.
    esc_output[2] = receiver_input_channel[3];                                          //Set the pulse for motor 2 equal to the throttle channel.
    esc_output[3] = receiver_input_channel[3];                                          //Set the pulse for motor 3 equal to the throttle channel.
    esc_output[4] = receiver_input_channel[3];                                          //Set the pulse for motor 4 equal to the throttle channel.
    esc_pulse_output();                                                                               //Send the ESC control pulses.
  }
  ...
  if(data == 'r'){
    ...
    receiver_input_channel[1] = convert_receiver_channel(1);                 //Convert the actual receiver signals for pitch to the standard
    receiver_input_channel[2] = convert_receiver_channel(2);                 //Convert the actual receiver signals for roll to the standard.
    receiver_input_channel[3] = convert_receiver_channel(3);                 //Convert the actual receiver signals for throttle to the standard   
    receiver_input_channel[4] = convert_receiver_channel(4);                 //Convert the actual receiver signals for yaw to the standard

    if(loop_counter == 125){                                                            //Print the receiver values every 0.5 s.
    ...
    if(start == 1 && receiver_input_channel[3] < 1050 && receiver_input_channel[4] > 1450)start = 2;
  ...
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5'){          //If motor 1, 2, 3 or 4 is selected by the user.
    ...
    if(new_function_request == true && loop_counter == 250){                            //Wait for the throttle to be set to 0.
      ...
      Serial.println(receiver_input_channel[3]);                                        //Print the actual throttle position.
      loop_counter = 0;                                                                 //Reset the loop_counter variable.
    }
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      receiver_input_channel[3] = convert_receiver_channel(3);          //Convert the actual receiver signals to the standard 1000 - 2000us.
      if(data == '1' || data == '5')esc_output[1] = receiver_input_channel[3];   //If motor 1 is requested set the pulse for motor 1
      else esc_output[1] = 1000;                                                        //If motor 1 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '2' || data == '5')esc_output[2] = receiver_input_channel[3];          //If motor 2 is requested set the pulse for motor 2
      else esc_output[2] = 1000;                                                        //If motor 2 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '3' || data == '5')esc_output[3] = receiver_input_channel[3];          //If motor 3 is requested set the pulse for motor 3.
      else esc_output[3] = 1000;                                                        //If motor 3 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '4' || data == '5')esc_output[4] = receiver_input_channel[3];          //If motor 4 is requested set the pulse for motor 4.
      else esc_output[4] = 1000;                                                        //If motor 4 is not requested set the pulse for the ESC to 1000us (off).
      ...
      for(start = 16; start > 0; start--){                                              //Do this loop 16 times to create an array of acc vectors.
      ...
      if(vibration_counter < 20){                                                       //If the vibration_counter is less than 20 do this.
       ...
       //Add the absolute difference between the average vector and current vector to the vibration_total_result variable.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      if(eeprom_data[31] == 2){
        ...
        //0.000001066 = 0.0000611 * (3.142(PI) / 180 degr) The Arduino sin function is in radians
        angle_pitch -= angle_roll * sin(gyro_yaw * 0.000001066);            //If the IMU has yawed transfer the roll angle to the pitch angle.
        angle_roll += angle_pitch * sin(gyro_yaw * 0.000001066);           //If the IMU has yawed transfer the pitch angle to the roll angle.
      }
      ...
      if(!first_angle){
        ...
        angle_pitch = angle_pitch * 0.9996 + angle_pitch_acc * 0.0004;   //Correct the drift of the gyro pitch angle with the acc pitch angle.
        angle_roll = angle_roll * 0.9996 + angle_roll_acc * 0.0004;            //Correct the drift of the gyro roll angle with the acc roll angle.
      }
...
ISR(PCINT0_vect){
  ...
  if(PINB & B00000001){                                              //Is input 8 high?
    ...
    receiver_input_pin[1] = current_time - timer[1];        //Channel 1 is current_time - timer[1].
  }
  //Channel 2=========================================
  if(PINB & B00000010 ){                                             //Is input 9 high?
    ...
    receiver_input_pin[2] = current_time - timer[2];        //Channel 2 is current_time - timer[2].
  }
  //Channel 3=========================================
  if(PINB & B00000100 ){                                             //Is input 10 high?
    ...
    receiver_input_pin[3] = current_time - timer[3];       //Channel 3 is current_time - timer[3].
  }
  //Channel 4=========================================
  if(PINB & B00001000 ){                                             //Is input 11 high?
    ...
    receiver_input_pin[4] = current_time - timer[4];      //Channel 4 is current_time - timer[4].
  }
}
...
void wait_for_receiver(){
  ...
  while(zero < 15){                                                             //Stay in this loop until the 4 lowest bits are set
    //Set bit 0 if the receiver pulse 1 is within the 900 - 2100 range
    if(receiver_input_pin[1] < 2100 && receiver_input_pin[1] > 900)zero |= 0b00000001; 
    //Set bit 1 if the receiver pulse 2 is within the 900 - 2100 range
    if(receiver_input_pin[2] < 2100 && receiver_input_pin[2] > 900)zero |= 0b00000010;
    //Set bit 2 if the receiver pulse 3 is within the 900 - 2100 range
    if(receiver_input_pin[3] < 2100 && receiver_input_pin[3] > 900)zero |= 0b00000100;
    //Set bit 3 if the receiver pulse 4 is within the 900 - 2100 range
...
int convert_receiver_channel(byte function){
  ...
  actual = receiver_input_pin[channel];                                        //Read the actual receiver value for the corresponding function
  ...
  high = (eeprom_data[channel * 2 + 7] << 8) | eeprom_data[channel * 2 + 6];   //Store the high value
...
void print_signals(){
  ...
  Serial.print("  Roll:");
  if(receiver_input_channel[1] - 1480 < 0)Serial.print("<<<");
  else if(receiver_input_channel[1] - 1520 > 0)Serial.print(">>>");
  else Serial.print("-+-");
  Serial.print(receiver_input_channel[1]);

  Serial.print("  Pitch:");
  if(receiver_input_channel[2] - 1480 < 0)Serial.print("^^^");
  else if(receiver_input_channel[2] - 1520 > 0)Serial.print("vvv");
  else Serial.print("-+-");
  Serial.print(receiver_input_channel[2]);

  Serial.print("  Throttle:");
  if(receiver_input_channel[3] - 1480 < 0)Serial.print("vvv");
  else if(receiver_input_channel[3] - 1520 > 0)Serial.print("^^^");
  else Serial.print("-+-");
  Serial.print(receiver_input_channel[3]);

  Serial.print("  Yaw:");
  if(receiver_input_channel[4] - 1480 < 0)Serial.print("<<<");
  else if(receiver_input_channel[4] - 1520 > 0)Serial.print(">>>");
  else Serial.print("-+-");
  Serial.println(receiver_input_channel[4]);
}

void esc_pulse_output(){
  zero_timer = micros();
  PORTD |= B11110000;                                                        //Set port 4, 5, 6 and 7 high at once
  timer_channel[1] = esc_output[1] + zero_timer;                 //Calculate the time when digital port 4 is set low.
  timer_channel[2] = esc_output[2] + zero_timer;                 //Calculate the time when digital port 5 is set low.
  timer_channel[3] = esc_output[3] + zero_timer;                 //Calculate the time when digital port 6 is set low.
  timer_channel[4] = esc_output[4] + zero_timer;                 //Calculate the time when digital port 7 is set low.

  while(PORTD >= 16){                                            //Execute the loop until digital port 4 to 7 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel[1] <= esc_loop_timer)PORTD &= B11101111;    //When the delay time is expired, digital port 4 is set low.
    if(timer_channel[2] <= esc_loop_timer)PORTD &= B11011111;    //When the delay time is expired, digital port 5 is set low.
    if(timer_channel[3] <= esc_loop_timer)PORTD &= B10111111;    //When the delay time is expired, digital port 6 is set low.
    if(timer_channel[4] <= esc_loop_timer)PORTD &= B01111111;    //When the delay time is expired, digital port 7 is set low.
  }
}
