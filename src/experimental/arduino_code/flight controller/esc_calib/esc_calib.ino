...
void loop(){
  ...
  if(receiver_input_channel[3] < 1025)new_function_request = false;                     //If the throttle is in the lowest position set the request flag to false.
  ...
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5'){          //If motor 1, 2, 3 or 4 is selected by the user.
    ...
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      receiver_input_channel[3] = convert_receiver_channel(3);                          //Convert the actual receiver signals for throttle to the standard 1000 - 2000us.
      if(data == '1' || data == '5')esc_output[1] = receiver_input_channel[3];          //If motor 1 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_output[1] = 1000;                                                        //If motor 1 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '2' || data == '5')esc_output[2] = receiver_input_channel[3];          //If motor 2 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_output[2] = 1000;                                                        //If motor 2 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '3' || data == '5')esc_output[3] = receiver_input_channel[3];          //If motor 3 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_output[3] = 1000;                                                        //If motor 3 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '4' || data == '5')esc_output[4] = receiver_input_channel[3];          //If motor 4 is requested set the pulse for motor 1 equal to the throttle channel.
...
void wait_for_receiver(){
  ...
  while(zero < 15){                                                             //Stay in this loop until the 4 lowest bits are set
    if(receiver_input_pin[1] < 2100 && receiver_input_pin[1] > 900)zero |= 0b00000001;  //Set bit 0 if the receiver pulse 1 is within the 900 - 2100 range
    if(receiver_input_pin[2] < 2100 && receiver_input_pin[2] > 900)zero |= 0b00000010;  //Set bit 1 if the receiver pulse 2 is within the 900 - 2100 range
    if(receiver_input_pin[3] < 2100 && receiver_input_pin[3] > 900)zero |= 0b00000100;  //Set bit 2 if the receiver pulse 3 is within the 900 - 2100 range
    if(receiver_input_pin[4] < 2100 && receiver_input_pin[4] > 900)zero |= 0b00001000;  //Set bit 3 if the receiver pulse 4 is within the 900 - 2100 range
