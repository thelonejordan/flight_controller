...
class Kalman_IMU{
	public:
		...
		void predict(){
			...
			predCov[0][1] = 0;//Cov[0][1] + dt*Cov[1][1]                              + Q[0][1];
			predCov[1][0] = 0;//Cov[1][0] + dt*Cov[1][1]                              + Q[1][0];
		...
		void update(){
			...
			rate_diff = rate - pred_rate;

			S[0][0] = predCov[0][0] + R[0][0];
			S[0][1] = 0;//predCov[0][1] + R[0][1];
			S[1][0] = 0;//predCov[1][0] + R[1][0];
			S[1][1] = predCov[1][1] + R[1][1];

			double divisor = S[0][0]*S[1][1] - S[0][1]*S[1][0];
		
			K[0][0] = (predCov[0][0]*S[1][1] - predCov[0][1]*S[1][0])/divisor;
			K[0][1] = 0;//(predCov[0][1]*S[0][0] - predCov[0][0]*S[0][1])/divisor;
			K[1][0] = 0;//(predCov[1][0]*S[1][1] - predCov[1][1]*S[1][0])/divisor;
			...
			Cov[0][1] = 0;//(1 - K[0][0])*predCov[0][1] + K[0][1]*predCov[1][1];
			Cov[1][0] = 0;//K[1][0]*predCov[0][0] + (1 - K[1][1])*predCov[1][0];
		...
		void init_processCov(double pred_var_angle, double pred_var_rate){
			...
			Q[0][1] = 0;//sqrt(abs(pred_var_angle*pred_var_rate));
			Q[1][0] = 0;//Q[0][1];
			Q[1][1] = pred_var_rate;
		}
